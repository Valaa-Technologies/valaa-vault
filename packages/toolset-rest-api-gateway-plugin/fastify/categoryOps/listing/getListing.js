...
export async function preload (server: RestAPIServer, route: Route) {
  ...
  route.vRoot = server.getEngine().getVrapper([
    connection.getPartitionRawId(), { partition: String(connection.getPartitionURI()) },
  ]);
  const listKuery = ["§->"];
  if (!server._extractValOSSchemaKuery(route.config.valos, listKuery)) {
...
export function createHandler (server: RestAPIServer, route: Route) {
  const kuery = ["§->"];
  server._buildKuery({ ...route.schema.response[200], valos: route.config.valos }, kuery);
  ...
  return (request, reply) => {
    ...
    server.logEvent(1, () => [`listing GET <${route.url}>`,
    ...
    if (fields) {
      results = server._pickResultsFields(results, fields);
    }
